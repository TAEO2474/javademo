...
//2. reference data type : arr (배열), class, interface, enum(상수)
//
//[argument 전달방식]
...
public class Java058_method {
	...
	public static void callByValue(int x, int y) {
		int temp; //"temporary 자바에 특별한 예약어나 문법은 아니고, 개발자가 임시로 값을 저장할 때 쓰는 변수 이름/ 이름 마음대로 변경 가능
		temp = x;
		x = y;
		y = temp;
	} // end callByValue()
