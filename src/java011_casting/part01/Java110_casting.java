...
//int num = 3;
//double data = num; //묵시적 형변환 (작 -> 큰)
//short val =(short)num;//명시적 형변환 (큰 -> 작)

//* 객체형변환 - 참조데이터 타입
//1. 상속관계일때 형변환이 가능하다.(is a)
//2 업캐스팅 발생된 후 다운캐스팅을 할 수 있다.
//- 업캐스팅(up-casting) (자식 → 부모) :  부모객체로 자식객체를 참조할도록 형변환하는 기술
//- 다운캐스팅(down-casting)  (부모 → 자식): 업캐스팅을 다시 원상복귀해주는 형변환하는 기술
//
...
public class Java110_casting {
	...
	public static void main(String[] args) {
		...
		//업캐스팅 후, toString을 호출하면, object를 toString() 검색 후 있으면
		//String에서 toString() 오버라이딩이 여부를 물어서 오버라이딩이 되어 있으면 
		//제어권을String으로 넘겨서toString() 메소드를 실행한다
	}
